...
const BADGES = [
  { id: "first-step", icon: "🏃", name: "First Step", description: "Selesaikan aktivitas pertama.", check: s => s.activities >= 1, progress: s => `${Math.min(s.activities, 1)} / 1 activity` },
  { id: "active-5", icon: "⚡", name: "Getting Active", description: "Selesaikan 5 aktivitas.", check: s => s.activities >= 5, progress: s => `${Math.min(s.activities, 5)} / 5 activities` },
  { id: "active-10", icon: "🔥", name: "On Fire", description: "Selesaikan 10 aktivitas.", check: s => s.activities >= 10, progress: s => `${Math.min(s.activities, 10)} / 10 activities` },
  { id: "active-25", icon: "💎", name: "25 Strong", description: "Selesaikan 25 aktivitas.", check: s => s.activities >= 25, progress: s => `${Math.min(s.activities, 25)} / 25 activities` },
  { id: "bingo-explorer", icon: "🧭", name: "Bingo Explorer", description: "Berpartisipasi di Bingo A, B, dan C.", check: s => s.variants.size >= 3, progress: s => `${s.variants.size} / 3 Bingo` },
  { id: "week-warrior", icon: "📅", name: "Week Warrior", description: "Aktif pada Week 1 sampai Week 4.", check: s => s.weeks.size >= 4, progress: s => `${s.weeks.size} / 4 weeks` },
  { id: "evidence-hero", icon: "📸", name: "Evidence Hero", description: "Upload evidence untuk 5 aktivitas.", check: s => s.evidence >= 5, progress: s => `${Math.min(s.evidence, 5)} / 5 evidence` },
  { id: "quiz-master", icon: "🧠", name: "Quiz Master", description: "Selesaikan Quiz ITGH 3 kali.", check: s => s.quizzes >= 3, progress: s => `${Math.min(s.quizzes, 3)} / 3 quizzes` }
];
...
function buildStats(rows) {
  ...
  return {
    ...
    variants: new Set(activities.map(row => String(row.variant || "").toUpperCase()).filter(Boolean)),
    weeks: new Set(rows.map(row => Number(row.week)).filter(week => Number.isInteger(week) && week > 0)),
    evidence: activities.filter(row => Boolean(row.evidenceUrl)).length,
    quizzes: rows.filter(row => Number(row.challengeIndex) === -1).length
  };
}
